...
const draftKey = "proupiqr-quotation-draft";
...
export function QuotationGenerator() {
  ...
  useEffect(() => {
    localStorage.setItem(draftKey, JSON.stringify({ merchant, customer, quoteNo, quoteDate, validUntil, gstPercent, discount, upiId, terms, preparedBy, items }));
  }, [merchant, customer, quoteNo, quoteDate, validUntil, gstPercent, discount, upiId, terms, preparedBy, items]);
  ...
  function safeFileName() {
    return (quoteNo || "quotation").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
  }

  async function renderPaper() {
    const el = paperRef.current;
    if (!el) throw new Error("Quotation preview not ready");
  ...
  async function shareOnWhatsapp() {
    ...
    try {
      setShareState("busy");
      const message = `*Quotation ${quoteNo} from ${merchant || "Merchant"}*\n` +
        `----------------------------\n` +
        `*For:* ${customer}\n` +
      ...
      if (navigator.share && navigator.canShare) {
        try {
          ...
          if (navigator.canShare({ files: [file] })) {
            await navigator.share({ files: [file], title: `Quotation ${quoteNo}`, text: message });
            shared = true;
          }
        } catch (err) {
