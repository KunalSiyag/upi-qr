    ...
    <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
      <div className="no-print rounded-[2rem] border border-white/75 bg-white/90 p-5 shadow-[0_18px_48px_rgba(17,59,44,0.08)]">
        ...
        <div className="flex flex-wrap gap-3 sm:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-leaf">{tr("Wealth planner")}</p>
            <h2 className="mt-1 text-2xl font-black text-forest">{tr("Project Your SIP Growth")}</h2>
          </div>
          <div className="flex flex-wrap gap-2 pb-1">
            <button onClick={downloadPdf} disabled={pdfState === "busy"} className="rounded-full bg-forest px-4 py-2 text-xs font-bold text-white hover:bg-leaf disabled:opacity-50 transition">{pdfState === "busy" ? tr("Generating...") : `${"📄"} ${tr("Projection PDF")}`}</button>
        ...
        <div className="mt-6 grid gap-4">
          <label className="text-sm font-bold text-forest">{tr("Monthly investment ₹")}<input type="number" value={monthly} onChange={(e) => setMonthly(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("Expected annual return %")}<input type="number" step="0.5" value={rate} onChange={(e) => setRate(e.target.value)} placeholder="Equity ~12, Debt ~7" className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("Investment period — years")}<input type="number" min={1} max={50} value={years} onChange={(e) => setYears(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("Assume inflation % (optional)")}<input type="number" step="0.5" value={inflation} onChange={(e) => setInflation(e.target.value)} placeholder="~6 shows today's-money value" className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl bg-cream p-4"><p className="text-[10px] font-black uppercase tracking-widest text-forest/50">{tr("You invest")}</p><p className="mt-1 text-xl font-black text-forest">{money(calc.invested)}</p></div>
          <div className="rounded-2xl bg-amber-100 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-forest/55">{tr("Wealth gained")}</p><p className="mt-1 text-xl font-black text-forest">{money(calc.gains)}</p></div>
          <div className="rounded-2xl bg-forest p-4 text-white"><p className="text-[10px] font-black uppercase tracking-widest text-white/60">{tr("Maturity value")}</p><p className="mt-1 text-2xl font-black">{money(calc.futureValue)}</p></div>
        </div>
      ...
      <article ref={paperRef} className="invoice-paper mx-auto h-fit w-full max-w-[780px] rounded-[2rem] border border-forest/10 bg-white p-6 shadow-[0_24px_80px_rgba(17,59,44,0.12)] md:p-8">
        <header className="flex items-start justify-between border-b-2 border-forest pb-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.24em] text-leaf">{tr("SIP growth projection")}</p>
          ...
          <div className="rounded-xl bg-mint p-3 text-right">
            <p className="text-[10px] font-bold text-forest/60">{tr("Maturity value")}</p>
        ...
        <table className="mt-5 w-full text-left text-sm">
          <thead><tr className="border-b border-forest/20 text-[10px] uppercase tracking-widest text-forest/55"><th className="py-2">{tr("Year")}</th><th className="py-2 text-right">{tr("Total invested")}</th><th className="py-2 text-right">{tr("Portfolio value")}</th><th className="py-2 text-right">{tr("Gains")}</th></tr></thead>
          <tbody>
            {calc.yearly.map((row) => (
