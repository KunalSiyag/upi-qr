...
export function DebitNoteGenerator({ lang = "en" }: { lang?: DocLang } = {}) {
  ...
  return (<div className="grid gap-8 lg:grid-cols-[0.92fr_1.08fr]">
    <div className="no-print rounded-[2rem] border border-white/75 bg-white/90 p-5 shadow-[0_18px_48px_rgba(17,59,44,0.08)]">
      ...
      <div className="flex flex-wrap gap-3 sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-leaf">{tr("GST debit note")}</p><h2 className="mt-1 text-2xl font-black text-forest">{tr("Debit Note (Section 34)")}</h2></div><div className="flex flex-wrap gap-2"><button onClick={downloadPng} disabled={pngState === "busy"} className="rounded-full border border-forest/15 px-4 py-2 text-xs font-bold text-forest hover:border-leaf transition">{pngState === "busy" ? "..." : tr("Export PNG")}</button><button onClick={downloadPdf} disabled={pdfState === "busy"} className="rounded-full bg-forest px-4 py-2 text-xs font-bold text-white hover:bg-leaf disabled:opacity-50 transition">{pdfState === "busy" ? "..." : tr("Export PDF")}</button></div></div>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <label className="text-sm font-bold text-forest">{tr("Supplier (your business)")}<input value={supplierName} onChange={e => setSupplierName(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest">{tr("Your GSTIN")}<input value={supplierGstin} onChange={e => setSupplierGstin(e.target.value)} placeholder="22AAAAA0000A1Z5" className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        ...
        <label className="text-sm font-bold text-forest">{tr("Buyer GSTIN")}<input value={buyerGstin} onChange={e => setBuyerGstin(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest sm:col-span-2">{tr("Buyer address")}<input value={buyerAddress} onChange={e => setBuyerAddress(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest">{tr("Debit note number")}<input value={dnNo} onChange={e => setDnNo(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest">{tr("Date")}<input type="date" value={dnDate} onChange={e => setDnDate(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className={`text-sm font-bold ${missingRef ? "text-red-600" : "text-forest"}`}>{tr("Original invoice ref")}<input value={origInvoice} onChange={e => setOrigInvoice(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest">{tr("Original invoice date")}<input type="date" value={origDate} onChange={e => setOrigDate(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest sm:col-span-2">{tr("Reason for debit note")}<textarea value={reason} onChange={e => setReason(e.target.value)} rows={2} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest sm:col-span-2">{tr("Additional note")}<textarea value={note} onChange={e => setNote(e.target.value)} rows={2} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
      </div>
      ...
      <div className="mt-5"><div className="flex items-center justify-between mb-2"><h3 className="text-xs font-black text-forest uppercase">{tr("Additional charges")}</h3><button onClick={addItem} className="text-[11px] font-bold text-leaf hover:text-forest">{tr("+ Add row")}</button></div>
        {items.map((it, idx) => (<div key={it.id} className="mb-2 grid grid-cols-5 gap-1.5 rounded-xl bg-cream/60 p-2">
          <input value={it.name} onChange={e => upd(it.id, "name", e.target.value)} placeholder={tr("Item")} className="col-span-2 rounded-lg border border-forest/10 bg-white px-2 py-1.5 text-xs font-medium outline-none focus:border-leaf" />
          <input value={it.qty} onChange={e => upd(it.id, "qty", e.target.value)} placeholder={tr("Qty")} className="rounded-lg border border-forest/10 bg-white px-2 py-1.5 text-xs font-medium outline-none focus:border-leaf" />
          <input value={it.rate} onChange={e => upd(it.id, "rate", e.target.value)} placeholder={tr("Rate")} className="rounded-lg border border-forest/10 bg-white px-2 py-1.5 text-xs font-medium outline-none focus:border-leaf" />
    ...
    <article ref={paperRef} className="invoice-paper mx-auto h-fit w-full max-w-[780px] rounded-[2rem] border border-forest/10 bg-white p-6 shadow-[0_24px_80px_rgba(17,59,44,0.12)] md:p-8">
      <header className="flex items-start justify-between border-b-2 border-forest pb-4">
        <div><p className="text-xs font-black uppercase tracking-[0.24em] text-leaf">{tr("Debit note")}</p><h2 className="mt-2 text-2xl font-black text-forest">{dnNo}</h2><p className="text-xs font-semibold text-forest/60">{tr("Date")}: {dnDate}</p></div>
      ...
      <section className="mt-4 grid grid-cols-2 gap-4 text-xs leading-6">
        <div><p className="font-black text-forest">{tr("Supplier")}</p><p className="font-semibold">{supplierName || "—"}</p>{supplierGstin && <p className="text-forest/60">GSTIN: {supplierGstin}</p>}<p className="text-forest/60">{supplierAddress}</p></div>
        <div className="text-right"><p className="font-black text-forest">{tr("Buyer")}</p><p className="font-semibold">{buyerName || "—"}</p>{buyerGstin && <p className="text-forest/60">GSTIN: {buyerGstin}</p>}<p className="text-forest/60">{buyerAddress}</p></div>
      </section>
      ...
      <div className="mt-5 overflow-x-auto"><table className="w-full text-xs">
        <thead><tr className="border-b-2 border-forest text-left"><th className="py-2 font-black">#</th><th className="py-2 font-black">{tr("Description")}</th><th className="py-2 text-right font-black">{tr("Qty")}</th><th className="py-2 text-right font-black">{tr("Rate")}</th><th className="py-2 text-right font-black">GST%</th><th className="py-2 text-right font-black">{tr("Amount")}</th></tr></thead>
