...
export function CashMemoGenerator({ lang = "en" }: { lang?: DocLang } = {}) {
  ...
  return (<div className="grid gap-8 lg:grid-cols-[0.92fr_1.08fr]">
    <div className="no-print rounded-[2rem] border border-white/75 bg-white/90 p-5 shadow-[0_18px_48px_rgba(17,59,44,0.08)]">
      ...
      <div className="flex flex-wrap gap-3 sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-leaf">{tr("Cash memo")}</p><h2 className="mt-1 text-2xl font-black text-forest">{tr("Retail billing slip")}</h2></div><button onClick={downloadPdf} disabled={pdfState === "busy"} className="rounded-full bg-forest px-4 py-2 text-xs font-bold text-white hover:bg-leaf disabled:opacity-50 transition">{pdfState === "busy" ? "..." : tr("Export PDF")}</button></div>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <label className="text-sm font-bold text-forest">{tr("Shop name")}<input value={shopName} onChange={e => setShopName(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest">{tr("Phone")}<input value={shopPhone} onChange={e => setShopPhone(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest sm:col-span-2">{tr("Address")}<input value={shopAddress} onChange={e => setShopAddress(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest">{tr("Customer name (optional)")}<input value={customerName} onChange={e => setCustomerName(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest">{tr("Memo number")}<input value={cmNo} onChange={e => setCmNo(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest">{tr("Date")}<input type="date" value={cmDate} onChange={e => setCmDate(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        <label className="text-sm font-bold text-forest">{tr("Counter / staff")}<input value={counter} onChange={e => setCounter(e.target.value)} placeholder="Counter 1" className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
      </div>
      <div className="mt-5"><div className="flex items-center justify-between mb-2"><h3 className="text-xs font-black text-forest uppercase">{tr("Items")}</h3><button onClick={addItem} className="text-[11px] font-bold text-leaf hover:text-forest">{tr("+ Add row")}</button></div>
        {items.map((it, idx) => (<div key={it.id} className="mb-2 grid grid-cols-4 gap-1.5 rounded-xl bg-cream/60 p-2">
          ...
          <div className="flex items-center gap-1"><input value={it.rate} onChange={e => upd(it.id, "rate", e.target.value)} placeholder={tr("Rate")} className="w-full rounded-lg border border-forest/10 bg-white px-2 py-1.5 text-xs font-medium outline-none focus:border-leaf" />{items.length > 1 && <button onClick={() => removeItem(it.id)} className="text-red-500 text-xs font-bold">×</button>}</div>
        </div>))}
      </div>
    </div>
    <article ref={paperRef} className="invoice-paper mx-auto h-fit w-full max-w-[780px] rounded-[2rem] border border-forest/10 bg-white p-6 shadow-[0_24px_80px_rgba(17,59,44,0.12)] md:p-8">
      <header className="text-center border-b-2 border-forest pb-4">
        <h2 className="text-2xl font-black text-forest">{shopName || tr("Shop Name")}</h2>
        {shopAddress && <p className="text-[11px] text-forest/60">{shopAddress}</p>}
        {shopPhone && <p className="text-[11px] text-forest/60">{tr("Tel:")} {shopPhone}</p>}
      </header>
      <section className="mt-4 grid grid-cols-2 gap-3 text-xs">
        <div><p className="font-black">{tr("Cash Memo")}</p><p className="font-semibold">{cmNo}</p><p className="text-forest/60">{tr("Date")}: {cmDate}</p>{counter && <p className="text-forest/60">{tr("Counter:")} {counter}</p>}</div>
        {customerName && <div className="text-right"><p className="font-black">{tr("Customer")}</p><p className="font-semibold">{customerName}</p></div>}
      </section>
      <div className="mt-5 overflow-x-auto"><table className="w-full text-xs">
        <thead><tr className="border-b-2 border-forest text-left"><th className="py-2 font-black">#</th><th className="py-2 font-black">{tr("Item")}</th><th className="py-2 text-right font-black">{tr("Qty")}</th><th className="py-2 text-right font-black">{tr("Rate")}</th><th className="py-2 text-right font-black">{tr("Amount")}</th></tr></thead>
