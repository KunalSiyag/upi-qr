...
export function PurchaseOrderGenerator({ lang = "en" }: { lang?: DocLang } = {}) {
    ...
    <div className="grid gap-8 lg:grid-cols-[0.92fr_1.08fr]">
      <div className="no-print rounded-[2rem] border border-white/75 bg-white/90 p-5 shadow-[0_18px_48px_rgba(17,59,44,0.08)]">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-sm font-bold text-forest">{tr("Buyer GSTIN")}<input value={buyerGstin} onChange={e => setBuyerGstin(e.target.value)} placeholder="22AAAAA0000A1Z5" className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          ...
          <label className="text-sm font-bold text-forest">{tr("PO number")}<input value={poNo} onChange={e => setPoNo(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("PO date")}<input type="date" value={poDate} onChange={e => setPoDate(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("Delivery expected by")}<input type="date" value={deliveryBy} onChange={e => setDeliveryBy(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("Terms & conditions")}<textarea value={terms} onChange={e => setTerms(e.target.value)} rows={2} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        </div>
        <div className="mt-5"><div className="flex items-center justify-between mb-2"><h3 className="text-xs font-black text-forest uppercase">{tr("Items")}</h3><button onClick={addItem} className="text-[11px] font-bold text-leaf hover:text-forest">{tr("+ Add row")}</button></div>
          {items.map((it, idx) => (<div key={it.id} className="mb-2 grid grid-cols-5 gap-1.5 rounded-xl bg-cream/60 p-2">
            ...
            <input value={it.rate} onChange={e => updateItem(it.id, "rate", e.target.value)} placeholder={tr("Rate")} className="rounded-lg border border-forest/10 bg-white px-2 py-1.5 text-xs font-medium outline-none focus:border-leaf" />
      ...
      <article ref={paperRef} className="invoice-paper mx-auto h-fit w-full max-w-[780px] rounded-[2rem] border border-forest/10 bg-white p-6 shadow-[0_24px_80px_rgba(17,59,44,0.12)] md:p-8">
        <header className="flex items-start justify-between border-b-2 border-forest pb-4">
          <div><p className="text-xs font-black uppercase tracking-[0.24em] text-leaf">{tr("Purchase order")}</p><h2 className="mt-2 text-2xl font-black text-forest">{poNo}</h2><p className="text-xs font-semibold text-forest/60">{tr("Date")}: {poDate}</p></div>
        ...
        <section className="mt-4 grid grid-cols-2 gap-4 text-xs leading-6">
          <div><p className="font-black text-forest">{tr("From (Buyer)")}</p><p className="font-semibold">{buyerName || "—"}</p>{buyerGstin && <p className="text-forest/60">GSTIN: {buyerGstin}</p>}<p className="text-forest/60 whitespace-pre-wrap">{buyerAddress}</p></div>
          <div className="text-right"><p className="font-black text-forest">{tr("To (Supplier)")}</p><p className="font-semibold">{supplierName || "—"}</p>{supplierGstin && <p className="text-forest/60">GSTIN: {supplierGstin}</p>}</div>
        </section>
        <div className="mt-5 overflow-x-auto"><table className="w-full text-xs">
          <thead><tr className="border-b-2 border-forest text-left"><th className="py-2 font-black">#</th><th className="py-2 font-black">{tr("Item")}</th><th className="py-2 text-right font-black">{tr("Qty")}</th><th className="py-2 text-right font-black">{tr("Rate")}</th><th className="py-2 text-right font-black">GST%</th><th className="py-2 text-right font-black">{tr("Amount")}</th></tr></thead>
        ...
        <div className="mt-3 ml-auto w-60 space-y-1 text-xs">
          <div className="flex justify-between"><span className="font-semibold">{tr("Subtotal")}</span><strong>{money(calc.subtotal)}</strong></div>
          <div className="flex justify-between"><span className="font-semibold">GST</span><strong>{money(calc.tax)}</strong></div>
          <div className="flex justify-between border-t-2 border-forest pt-1 text-sm"><span className="font-black">{tr("Total")}</span><strong className="font-black">{money(calc.total)}</strong></div>
        </div>
        {deliveryBy && <div className="mt-4 rounded-xl bg-mint p-3 text-xs"><span className="font-black">{tr("Delivery expected by:")}</span> {deliveryBy}</div>}
        {terms && <p className="mt-3 text-[10px] leading-relaxed text-forest/60 border-t border-forest/10 pt-3">{terms}</p>}
        <footer className="mt-4 pt-3 border-t border-forest/10 text-[10px] text-forest/60"><p>{tr("This is a purchase order — not a tax invoice or contract. Prepared using Pro UPI QR.")}</p></footer>
      </article>
    </div>
  );
}