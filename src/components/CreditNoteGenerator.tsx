    ...
    <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
      <div className="no-print rounded-[2rem] border border-white/75 bg-white/90 p-5 shadow-[0_18px_48px_rgba(17,59,44,0.08)]">
        ...
        <div className="flex flex-wrap gap-3 sm:justify-between">
          <div className="border-b border-forest/5 pb-4 sm:w-auto sm:flex-1">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-leaf">{tr("GST credit note")}</p>
            <h2 className="mt-1 text-2xl font-black text-forest">{tr("Adjust A Taxed Sale")}</h2>
          </div>
          <div className="flex flex-wrap gap-2 pb-4">
            <button onClick={shareOnWhatsapp} className="rounded-full bg-[#25D366] px-4 py-2 text-xs font-bold text-white hover:bg-[#1da851] transition">💬 {tr("Share")}</button>
            <button onClick={downloadPdf} disabled={pdfState === "busy"} className="rounded-full bg-forest px-4 py-2 text-xs font-bold text-white hover:bg-leaf disabled:opacity-50 transition">{pdfState === "busy" ? tr("Generating...") : `${"📄"} ${tr("PDF")}`}</button>
            <button onClick={downloadPng} disabled={pngState === "busy"} className="rounded-full bg-mint px-4 py-2 text-xs font-bold text-forest hover:bg-leaf hover:text-white disabled:opacity-50 transition">{pngState === "busy" ? tr("Generating...") : `${"🖼️"} ${tr("PNG")}`}</button>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <label className="text-sm font-bold text-forest">{tr("Your business")}<input value={supplierName} onChange={(e) => setSupplierName(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("Your GSTIN")}<input value={supplierGstin} onChange={(e) => setSupplierGstin(e.target.value.toUpperCase())} maxLength={15} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium uppercase outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest sm:col-span-2">{tr("Your address")}<input value={supplierAddress} onChange={(e) => setSupplierAddress(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("Customer")}<input value={recipientName} onChange={(e) => setRecipientName(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("Customer GSTIN (B2B)")}<input value={recipientGstin} onChange={(e) => setRecipientGstin(e.target.value.toUpperCase())} maxLength={15} placeholder="Leave blank for B2C" className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium uppercase outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("Credit note number")}<input value={noteNo} onChange={(e) => setNoteNo(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("Credit note date")}<input type="date" value={noteDate} onChange={(e) => setNoteDate(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          ...
          <label className="text-sm font-bold text-forest">{tr("Original invoice date")}<input type="date" value={origInvoiceDate} onChange={(e) => setOrigInvoiceDate(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
          <label className="text-sm font-bold text-forest">{tr("Reason for credit")}<select value={reason} onChange={(e) => setReason(e.target.value as (typeof reasons)[number])} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf">{reasons.map((r) => <option key={r} value={r}>{tr(r)}</option>)}</select></label>
          <div className="text-sm font-bold text-forest">
            Supply type
            <div className="mt-2 flex gap-2">
              <button type="button" onClick={() => setSupplyType("intra")} aria-pressed={supplyType === "intra"} className={`flex-1 rounded-xl border px-3 py-2.5 text-xs font-bold transition ${supplyType === "intra" ? "border-leaf bg-leaf text-white" : "border-forest/15 bg-cream text-forest"}`}>{tr("Intra-state")}<br /><span className="font-normal opacity-80">CGST + SGST</span></button>
              <button type="button" onClick={() => setSupplyType("inter")} aria-pressed={supplyType === "inter"} className={`flex-1 rounded-xl border px-3 py-2.5 text-xs font-bold transition ${supplyType === "inter" ? "border-leaf bg-leaf text-white" : "border-forest/15 bg-cream text-forest"}`}>{tr("Inter-state")}<br /><span className="font-normal opacity-80">IGST</span></button>
            </div>
          </div>
          <label className="text-sm font-bold text-forest sm:col-span-2">{tr("Signed off as")}<input value={signatory} onChange={(e) => setSignatory(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
        </div>

        <div className="mt-6 space-y-3">
          <div className="flex items-center justify-between"><h3 className="font-black text-forest">{tr("Items being credited")}</h3><button onClick={addItem} className="text-sm font-bold text-leaf">{tr("+ Add item")}</button></div>
          {items.map((item) => (
          ...
          <p className="text-[11px] font-semibold text-forest/50">{tr("Columns: description · qty · rate ₹ · GST %")}</p>
        </div>

        <label className="mt-4 block text-sm font-bold text-forest">{tr("Notes")}<textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} placeholder="Extra details for the reason above…" className="mt-2 w-full rounded-2xl border border-forest/10 bg-cream px-4 py-3 font-medium outline-none focus:border-leaf" /></label>
      </div>
      ...
      <article ref={paperRef} className="invoice-paper mx-auto h-fit w-full max-w-[820px] rounded-[2rem] border border-forest/10 bg-white p-6 shadow-[0_24px_80px_rgba(17,59,44,0.12)] md:p-9">
        <header className="flex flex-col gap-5 border-b-2 border-forest pb-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.24em] text-leaf">{tr("Credit note (GST)")}</p>
          ...
          <div className="rounded-2xl bg-sun/40 p-4 text-right">
            ...
            <p className="mt-1 text-xs font-semibold text-forest/65">{tr("Date")}: {noteDate}</p>
            <p className="text-xs font-semibold text-forest/65">{tr("Reason:")} {tr(reason)}</p>
          </div>
        </header>

        <section className="mt-5 grid gap-4 sm:grid-cols-2">
          <div className="rounded-2xl bg-cream p-4">
            <p className="text-xs font-black uppercase tracking-[0.18em] text-forest/50">{tr("Credited to")}</p>
          ...
          <div className={`rounded-2xl p-4 ${missingInvoiceRef ? "border-2 border-dashed border-red-300 bg-red-50" : "bg-cream"}`}>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-forest/50">{tr("Original invoice")}</p>
            ...
            {origInvoiceDate && <p className="text-xs font-semibold text-forest/65">{tr("Dated:")} {origInvoiceDate}</p>}
        ...
        <div className="mt-5 overflow-hidden rounded-2xl border border-forest/10">
          <table className="w-full text-left text-sm">
            <thead className="bg-mint text-xs uppercase tracking-[0.14em] text-forest/70"><tr><th className="p-3">{tr("Description")}</th><th className="p-3 text-right">{tr("Qty")}</th><th className="p-3 text-right">{tr("Rate")}</th><th className="p-3 text-right">{tr("GST %")}</th><th className="p-3 text-right">{tr("Taxable")}</th></tr></thead>
        ...
        <section className="mt-5 space-y-2 text-sm">
          <div className="flex justify-between"><span>{tr("Total taxable value reduced")}</span><strong>{money(totals.taxableTotal)}</strong></div>
          {supplyType === "intra" ? (
            <>
              <div className="flex justify-between"><span>{tr("CGST reduced")}</span><strong>{money(totals.taxTotal / 2)}</strong></div>
              <div className="flex justify-between"><span>{tr("SGST reduced")}</span><strong>{money(totals.taxTotal / 2)}</strong></div>
            </>
          ) : (
            <div className="flex justify-between"><span>{tr("IGST reduced")}</span><strong>{money(totals.taxTotal)}</strong></div>
          )}
          <div className="mt-2 flex justify-between border-t-2 border-forest pt-3 text-lg text-forest"><span className="font-black">{tr("Total credit value")}</span><strong>{money(totals.grandTotal)}</strong></div>
        </section>
